...
public class OpenApiConfig {
    ...
    public OpenAPI votingSystemOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("Sistema de Votación Electrónica API")
                        .description("API REST para sistema de votación electrónica con estructuras de datos personalizadas")
                        .version("1.0.0")
                        .contact(new Contact().name("Equipo de Desarrollo").email("dev@votingsystem.com").url("https://github.com/voting-system"))
                        .license(new License().name("MIT License").url("https://opensource.org/licenses/MIT")))
                .servers(List.of(
                        new Server().url("http://localhost:" + serverPort).description("Servidor de desarrollo"),
                        new Server().url("https://api.votingsystem.com").description("Servidor de producción")))
